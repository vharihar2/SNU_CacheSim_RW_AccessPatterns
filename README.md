...
- A poor-man's "GUI visualizer" is available by running a Python script on the above generated output text file. The script reads the text file generated by the above invocation of the cache simulator, and generates an Excel XLSX file which is much more readable. The script is run as follows (needs Python 3, it has been tested on Python v3.14.7):
...
- Navigate to the `test` directory. If on Windows, invoke a Git Bash terminal there. If on Linux, invoke a GNOME-Terminal there. In the respective terminal (as the case may be), run the following command:
    >`run_tests.sh`
...
- P4:
    ...
    - Keep increasing regression test cases (tcs*).
